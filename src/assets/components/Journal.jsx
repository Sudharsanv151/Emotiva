...
const Journals = () => {
  ...
  return (
    <div className="container mx-auto  py-8 max-w-4xl">
      <div className="bg-slate-800/40 rounded-xl p-6 ring-1 ring-slate-300 shadow-lg">
        <h1 className="text-3xl font-bold mb-6 text-white flex items-center">
          <BookOpen className="mr-3 text-blue-400" />
          Your Journals
        </h1>

        <div className="mb-6 bg-slate/10 p-4 rounded-lg">
          <input
            type="text"
            value={newJournal.title}
            onChange={(e) => setNewJournal({ ...newJournal, title: e.target.value })}
            placeholder="Journal Title"
            className="w-full bg-transparent border-b border-white/30 text-white mb-4 pb-2 focus:outline-none focus:border-blue-400"
          />
          <textarea
            value={newJournal.content}
            onChange={(e) => setNewJournal({ ...newJournal, content: e.target.value })}
            placeholder="Journal Content"
            className="w-full bg-transparent border-b border-white/30 text-white h-32 mb-4 pb-2 focus:outline-none focus:border-blue-400"
          />
        ...
        <h2 className="text-2xl font-bold mb-4 text-white">Previous Journals</h2>
        <div className="space-y-4">
          {journals.map((journal) => (
            <div 
              key={journal._id} 
              className="bg-white/10 backdrop-blur-md rounded-lg p-4 flex justify-between items-start hover:bg-white/20 transition"
            >
